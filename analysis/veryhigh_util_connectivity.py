...
def _helper_compute_multiband_connectivity(job):
    ...
    if(np.isnan(data_clip).any()):
        ...
        adj_veryhigh = veryhigh_multiband_conn(data_clip, Fs)

    return (epoch,adj_veryhigh)
    ...
    for event_type, events in data['PATIENTS'][patient_id]['Events'].items():
        for event_id in events.keys():
            ...
            if(os.path.isfile('%s/%s/aim3/%s.Ictal.%s.multiband.npz'%(comp_dir,patient_id,patient_id,event_id))):
                ...
                tmp = np.load('%s/%s/aim3/%s.Ictal.%s.multiband.npz'%(comp_dir,patient_id,patient_id,event_id))
                all_adj_alphatheta = tmp['all_adj_alphatheta']
                all_adj_beta = tmp['all_adj_beta']
                all_adj_lowgamma = tmp['all_adj_lowgamma']
                all_adj_highgamma = tmp['all_adj_highgamma']

                epoch_length = tmp['epoch_length']
                all_adj_broadband_CC = tmp['all_adj_broadband_CC']
                ...
                for epoch in range(start_epoch,end_epoch+1):
                    all_adj_veryhigh[:,:,epoch] = return_list[epoch-start_epoch][1]
